...
public class HashMap {
	 ...
	 * 
	 * If the current number of districts is 34. However, they expect their peak
	 * volume to grow to 52 districts nationwide. So you we can use table size of
	 * 52/0.8 = 65. The next prime number closest to that is 67. Using a table size
	 * of 67 will ensure that the table has enough space to store all the districts,
	 * even if the maximum number is reached, and it will also help to reduce the
	...
	public HashMap() {
		...
		districts = new District[TABLE_LENGTH];
	...
	public int hash(String key) {
		...
		while (districts[p] != null && !districts[p].getId().equals(key)) {
			i++;
			p = p + i * i;
	 ...
	 */
	public void put(District r) {
		int position = hash(r.getId());
		districts[position] = r;
		numElements++;
	...
	public District get(String key) {
		...
		District pr = districts[position];
		if (pr != null) {
			if (!pr.isActive()) {
				pr = null;
			}
	...
	public void remove(String key) {
		int position = hash(key);
		if (districts[position] != null) {
			districts[position].setActive(false);
		}
